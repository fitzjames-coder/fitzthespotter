...
async function overpassPost(query) {
  ...
  // First endpoint to respond successfully wins
...
export async function getAirportDiagram(airport) {
  ...
  // b–d) Fetch from Overpass (all network errors caught below)
  try {
...
out geom;`
    ...
    // All endpoints failed/timed out — do NOT cache, allow retry
